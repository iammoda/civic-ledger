...
async def sync_ontario_lobbying_job(ctx: dict[str, Any]) -> None:
    """Weekly: Ontario lobbyist registry registrations (incremental)."""
    from datetime import datetime, timezone

    from app.db.session import SessionLocal
    from app.ingestion.ontario_lobbying import sync_ontario_lobbying
    from app.models import IngestionRun

    db = SessionLocal()
    try:
        run = IngestionRun(source_name="oico", job_name="ontario_lobbying_sync", status="running")
        db.add(run)
        db.commit()
        try:
